...
function subirFoto (event, template, subseccionId) {
    ...
    if ('files' in archivo) {
        ...
        if (archivo.files.length == 0) {
          ...
          for (var i = 0; i < archivo.files.length; i++) {
            ...
            Archivos.insert(doc, function (err, fileObj) {
              if (err) {
...
Template.ListaDeSecciones.onCreated(function () {
	var self = this;
...
Template.ListaDeSecciones.helpers({
	secciones: function () {
		return Secciones.find();
	}
});

Template.FormularioSeccion.onCreated(function () {
	var self = this;
...
Template.formularioUsuario.onCreated(function () {
	var self = this;
...
Template.formularioUsuario.helpers({
	tipos: function () {
		return Tipos.find();
	}
});

Template.FormularioSeccion.onRendered(function () {
	$(function() {
        $('#cp3').colorpicker({
            color: '#AA3399',
            format: 'rgba'
...
Template.FormularioSeccion.helpers({
	tipos: function () {
		return Tipos.find();
	},
	secciones: function () {
...
Template.FormularioSeccion.events({
	'submit form': function (event, template) {
		event.preventDefault();
		let datos = {
			nombre: template.find("[name='seccion']").value,
		...
		Meteor.call('agregarSeccion', datos, function (err) {
			if (err) {
				console.log(err);
			} else {
				console.log('funca!');
	...
	'click #agregar-contenido': function (event, template) {
		event.preventDefault();

		let datos = {
			titulo: template.find("[name='titulo']").value,
			descripcion: template.find("#contenido").value,
			seccionId: $( "#seccio" ).val()
		}

		
		Meteor.call('agregarSubseccion', datos, function (err, result) {
			if (err) {
				console.log(err);
			} else {
